...
public class AWSSpeechSynthesizer {
    ...
    public AWSSpeechSynthesizer() {
        ...
        builder.setRegion((String) Optional.ofNullable(prop.get("region")).orElseThrow(() -> new IllegalArgumentException("No region is set, speech synthesizing is disabled")));
        builder.setCredentials(new AWSStaticCredentialsProvider(
                new BasicAWSCredentials(
                        (String) Optional.ofNullable(prop.get("accessKey")).orElseThrow(() -> new IllegalArgumentException("No access key is set, speech synthesizing is disabled")),
                        (String) Optional.ofNullable(prop.get("secretKey")).orElseThrow(() -> new IllegalArgumentException("No secret key is set, speech synthesizing is disabled"))
                )));
    ...
    private InputStream synthesizeSpeech(VoiceId voiceId, String text) {
        try {
            ...
            throw new IllegalArgumentException("Cannot create the MP3 stream.", e);
        }
    }
