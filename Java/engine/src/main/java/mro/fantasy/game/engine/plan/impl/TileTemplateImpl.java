...
public class TileTemplateImpl extends AbstractAudioGameResource implements TileTemplate {
    ...
    // @Override
    // public TileTemplate copy() {
    //     TileTemplateImpl newTT = new TileTemplateImpl();
    //
    //     newTT.type = this.type;
    //     newTT.name = this.name;
    //     newTT.description = this.description;
    //     newTT.size = new Size(this.size.columns(), this.size.rows());
    //     newTT.audioBundleName = this.audioBundleName;
    //     newTT.audioKey = this.audioKey;
    //     this.fields.stream().forEach(f -> newTT.fields.add((FieldImpl) f.copy()));
    //
    //     return newTT;
    // }
