...
import mro.fantasy.game.engine.plan.PlanRenderer;
...
public class ASCIIPlanRenderer implements PlanRenderer {
     ...
     */
    private void renderRowContent(Plan plan, StringBuffer buf, int row, int cellRow, String start, String middle, String end, List<RenderOptions> options) {
        ...
        for (int column = 0; column < plan.getSize().columns(); column++) {
            ...
            if (tile == null) {
                ...
                switch (cellRow) {
                    case 0:
                        Condition.of(field.isAnchor() && options.contains(RenderOptions.ANCHOR))
                                .ifTrue(() -> {
                        ...
                        break;
                    case 1:
                        Condition.of(options.contains(RenderOptions.TEMPLATE_NAME) || tile.getType().isCharacter())
                    ...
                    case 2:
                        ...
                        //     buf.append(" ".repeat(CELL_WIDTH - tile.getRotation().toDegreeString().length() - 2));
                        // } else {
                        buf.append(getIconString(field, CELL_WIDTH - 2));
                        // }
                        break;
                }
            }
     ...
     * @param buf    the buffer to append the data to
     */
    private void renderRowHeader(Plan plan, StringBuffer buf, String start, String middle, String end) {
        buf.append("   ");
        buf.append(start);
