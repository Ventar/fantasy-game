...
public class AudioResource implements GameResource {
    ...
    protected AudioResource(AudioResourceBundle bundle, String key, String ssml, String bundleHash, AudioResourceMP3FileResolver mp3StreamFunction) {
        ...
        if (!tmpSSML.startsWith("<speak>")) {                           // to be valid from AWS perspective every SSML text needs to start with the <speak> tag
            tmpSSML = "<speak>" + tmpSSML;
        }

        if (!tmpSSML.endsWith("</speak>")) {                            // and is closed.
            tmpSSML += "</speak>";
        }
    ...
    public InputStream getMP3Stream(AudioResource... variables) {
        ...
        ValidationUtils.requireFalse(variables == null
                                             || variables.length != ssml.split("\\{\\}", -1).length - 1,    // count the occurrence of {} substring in SSML text
        ...
        for (AudioResource part : split()) {   // split the SSML of this resource into multiple audio resources
            ...
            if (part.isVariablePlaceholder()) tmpStream = variables[placeholderCnt++].getMP3Stream();  // take the next variable stream and increase the placeholder counter
    ...
    private static boolean isEmptySSML(String ssml) {
        ...
        return tmpSSML.equals("<speak></speak>")
                       || tmpSSML.equals("<speak>")
                       || tmpSSML.equals("</speak>");
    }
